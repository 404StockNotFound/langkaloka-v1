  ...
  export default function ChatPage() {
    ...
    // ❌ kalau kita yg kirim → skip
    if (data.senderId === myId) return

    // ✅ AUTO READ langsung (INI FIX NYA)
    const token = localStorage.getItem("token")
    ...
    if (token) {
      ...
      // ✅ READ RECEIPT (FIX FINAL)
      channel.bind("read", (data: any) => {
        const myId = localStorage.getItem("userId")

        // ❌ skip kalau kita sendiri yg read
        if (data.readerId === myId) return

        // ✅ update PER MESSAGE (AKURAT)
        setMessages((prev) =>
          prev.map((msg) => {
        ...
        <div className="max-w-4xl mx-auto w-full p-6 flex flex-col gap-4">
          ...
          >
            {messages.map((msg) => {
              ...
              return (
                <div
                  key={`${msg.id}-${msg.createdAt}`} // 🔥 FIX DUPLICATE KEY
